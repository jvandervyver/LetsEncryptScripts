...
ENCODED_USERNAME=`urlEncode "$ENCODED_USERNAME"`
...
curl "https://${IPMI_HOSTNAME}/cgi/login.cgi" -vvvvv -skSL -X POST -H 'Content-Type: application/x-www-form-urlencoded' --data-raw "name=$ENCODED_USERNAME&pwd=$ENCODED_PASSWORD&check=00" -b "${COOKIES_FILE}" -c "${COOKIES_FILE}" -o "${CONTENT_FILE}"
cat "${CONTENT_FILE}"
